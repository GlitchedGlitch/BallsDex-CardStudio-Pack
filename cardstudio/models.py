...
class CardConfig(models.Model):
    title_font = models.FileField(null=True, blank=True, help_text="Title font (.ttf/.otf).")
    title_size = models.PositiveIntegerField(default=170)

    capacity_name_font = models.FileField(null=True, blank=True, help_text="Ability name font (.ttf/.otf).")
    capacity_name_size = models.PositiveIntegerField(default=110)

    capacity_description_font = models.FileField(
        null=True, blank=True, help_text="Ability description font (.ttf/.otf)."
    )
    capacity_description_size = models.PositiveIntegerField(default=75)

    stats_font = models.FileField(null=True, blank=True, help_text="Health/attack font.")
    stats_size = models.PositiveIntegerField(default=130)

    credits_font = models.FileField(null=True, blank=True, help_text="Credits font.")
    credits_size = models.PositiveIntegerField(default=40)

    rarity_font = models.FileField(null=True, blank=True, help_text="Rarity font.")
    ...
    def get_config(cls) -> CardConfig | None:
        """Return the configuration if one exists. No config means Card Studio is disabled.

        Synchronous: only safe to call from a thread with no running event loop (e.g. inside an
        executor, or a plain Django admin/WSGI view). Calling this directly from a coroutine on
        the bot's event loop raises SynchronousOnlyOperation, per BallsDex's own convention that
        Django queries must go through the async API in async contexts. Use `aget_config()` there.
        """
        try:
    ...
    @classmethod
    async def aget_config(cls) -> CardConfig | None:
        """Async-safe equivalent of `get_config()`, for use from bot commands and other coroutines."""
        try:
